...
def test_polygon():
    ...
    assert math.isclose(p.side_length, math.sqrt(2), rel_tol=rel_tol, abs_tol=abs_tol), f"actual: {p.side_length}, expected: {math.sqrt(2)}"
    ...
    assert math.isclose(p.apothem, 0.707, rel_tol=rel_tol, abs_tol=abs_tol), f"actual: {p.perimeter}, expected: 0.707"
